...
# NOTE - This part doesn't actually happen in the flow, it is just Ishaan messing around 
 open_input(){
  read -rsp $'Press any key to continue (ctrl-c to quit):\n' -n1 key

  if [[$key -eq 1]]; then
    xdg-open "https://$HOST/app/${organization_name}/workspaces/${workspace_name}"
